...
begin {
	...
	try {
		...
		if (!$EnabledNicIndex) {
			Write-Error "Computer '$Computername' does not have dynamic DNS enabled on any interface or on more than 1 interface"
			exit
		}
		
		$PrimaryDnsServer = Get-ClientPrimaryDns $EnabledNicIndex
		if (! $PrimaryDnsServer) {
			Write-Error "Could not find computer '$Computername' primary DNS server."
			exit
		}
		$DnsZone = Get-ClientPrimaryDnsSuffix
		if (! $DnsZone) {
			Write-Error "Could not find computer '$Computername' primary DNS suffix."
			exit
		}
		$script:ZoneAging = Get-DnsServerZoneAging -Name $DnsZone -ComputerName $PrimaryDnsServer
		
		$Record = Get-DnsServerResourceRecord -ComputerName $PrimaryDnsServer -Name $Computername -RRType A -ZoneName $DnsZone
		if (!(Validate-IsInRefreshPeriod $Record)) {
